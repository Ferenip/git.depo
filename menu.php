...
/**
 * MENÜ SİSTEMİ
 * JavaScript ile sekme (tab) geçişi sağlanarak sayfa yenilenmeden kategoriler arası geçiş yapılır.
 */
include 'header.php'; ?>
...
<section class="hero-section">
    <div class="hero-text">
        ...
        <div class="slider-indicators">
            <div class="indicator active" onclick="goToSlide(0)"><span>01</span>
                <div class="line"></div>
            </div>
            <div class="indicator" onclick="goToSlide(1)"><span>02</span>
                <div class="line"></div>
            </div>
            <div class="indicator" onclick="goToSlide(2)"><span>03</span>
                <div class="line"></div>
            </div>
            <div class="indicator" onclick="goToSlide(3)"><span>04</span>
                <div class="line"></div>
            </div>
        </div>
    </div>
...
<section class="products-grid" id="kahveler-grid">
    <?php
    // Kahveler kategorisindeki aktif ürünleri en son eklenenden geriye doğru çeker
    $kahve_sorgu = $db->prepare("SELECT * FROM urunler WHERE kategori = 'kahveler' AND (durum = 1 OR durum IS NULL) ORDER BY id DESC");
    $kahve_sorgu->execute();
...
<section class="products-grid" id="kupalar-grid" style="display:none;">
    ...
    $kupa_sorgu = $db->prepare("SELECT * FROM urunler WHERE kategori = 'kupalar' AND (durum = 1 OR durum IS NULL) ORDER BY id DESC");
    $kupa_sorgu->execute();
