<?php
/**
 * KURUMSAL SAYFASI (Hakkımızda & Mağazalar)
 * Sayfalandırma (Pagination) algoritması kullanılarak büyük verilerin sunucuyu yorması engellenmiştir.
 */
...
<div class="kurumsal-container">
    <?php if ($tab == 'magazalar'): ?>
        ...
        // SQL sorgusunda kaçıncı kayıttan başlanacağını hesaplar (Örn: 2. sayfa için 8. kayıttan başla)
        ...
        $subeler = $db->query("SELECT * FROM subeler ORDER BY il ASC, ilce ASC LIMIT $baslangic, $limit")->fetchAll(PDO::FETCH_ASSOC);
        ?>
        ...
        <div class="stores-grid">
            <?php if (count($subeler) > 0): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p style="grid-column: 1 / -1; text-align: center; font-size: 1.2rem; color: var(--text-gray);">Henüz eklenmiş
                    bir mağaza bulunmamaktadır.</p>
            <?php endif; ?>
        </div>
        ...
        <?php if ($toplam_sayfa > 1): ?>
            <div class="pagination">
                ...
                <?php for ($i = 1; $i <= $toplam_sayfa; $i++): ?>
                    <a href="?tab=magazalar&page=<?php echo $i; ?>"
                        class="page-btn <?php echo ($i == $sayfa) ? 'active' : ''; ?>"><?php echo $i; ?></a>
                <?php endfor; ?>
